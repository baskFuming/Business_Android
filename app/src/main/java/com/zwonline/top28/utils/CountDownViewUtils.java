...
public class CountDownViewUtils extends View {
    ...
    public CountDownViewUtils(Context context) {
        this(context, null);
    }

    public CountDownViewUtils(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CountDownViewUtils(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;

        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.CountDownViewUtils);
        mRetreatType = array.getInt(R.styleable.CountDownViewUtils_cd_retreat_type, 1);
        location = array.getInt(R.styleable.CountDownViewUtils_cd_location, 1);
        mCircleRadius = (int) array.getDimension(R.styleable.CountDownViewUtils_cd_circle_radius, dip2px(context, 25));//默认25dp
        mPaintArcWidth = array.getDimension(R.styleable.CountDownViewUtils_cd_arc_width, dip2px(context, 3));//默认3dp
        mPaintArcColor = array.getColor(R.styleable.CountDownViewUtils_cd_arc_color, mPaintArcColor);
        mTextSize = (int) array.getDimension(R.styleable.CountDownViewUtils_cd_text_size, dip2px(context, 14));//默认14sp
        mTextColor = array.getColor(R.styleable.CountDownViewUtils_cd_text_color, mTextColor);
        mPaintBackGroundColor = array.getColor(R.styleable.CountDownViewUtils_cd_bg_color, mPaintBackGroundColor);
        mLoadingTime = array.getInteger(R.styleable.CountDownViewUtils_cd_animator_time, 3);//默认3秒
        mLoadingTimeUnit = array.getString(R.styleable.CountDownViewUtils_cd_animator_time_unit);//时间单位
