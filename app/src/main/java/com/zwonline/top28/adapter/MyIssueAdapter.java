 ...
 * @date 2017/12/25
 */
public class MyIssueAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    private List<MyIssueBean.DataBean> list;
    private Context context;

    public MyIssueAdapter(List<MyIssueBean.DataBean> list, Context context) {
        this.list = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(context).inflate(R.layout.myissue_item,parent,false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        Glide.with(context).load(list.get(position).path).into(myViewHolder.image);
